...
class DIT(nn.Module):
    def __init__(self, im_size, im_channels, config):
        ...
        self.cond_proj = nn.Sequential(
            nn.Linear(208, self.hidden_size),
            nn.SiLU(),
            nn.Linear(self.hidden_size, self.hidden_size)
        )
        ...
        nn.init.normal_(self.cond_proj[0].weight, std=0.02)
        nn.init.normal_(self.cond_proj[2].weight, std=0.02)
    ...
    def forward(self, x, t, volt):

        # concatentate condition with input
        #volt_inter = F.interpolate(volt.view(-1, 1, 13, 16), size=(32, 32), mode="nearest")
        #x = torch.cat([x, volt_inter], dim=1)
        # Patchify
        ...
        volt_emb = self.cond_proj(volt)
        tv_emb_adaln = t_emb  + volt_emb

        # Go through the transformer layers
        for layer in self.layers:
            out = layer(out, tv_emb_adaln,volt_cross_emb=None) # None for cross-attn, if not None pass volt_emb

        # Shift and scale predictions for output normalization
        pre_mlp_shift, pre_mlp_scale = self.adaptive_norm_layer(tv_emb_adaln).chunk(2, dim=1)
